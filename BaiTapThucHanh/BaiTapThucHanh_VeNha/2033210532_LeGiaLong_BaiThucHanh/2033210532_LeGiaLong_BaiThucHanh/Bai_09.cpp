...
int main() {
	...
	// Nhập giá trị cho a với kiểm tra điều kiện
	printf("Nhap so thuc a: ");
	while (scanf("%f", &a) != 1) {
		// Xóa bộ đệm đầu vào để tránh vòng lặp vô hạn
		while (getchar() != '\n');
		printf("Loi: Vui long nhap mot so thuc hop le cho a: ");
	}

	// Nhập giá trị cho b với kiểm tra điều kiện
	printf("Nhap so thuc b: ");
	while (scanf("%f", &b) != 1) {
		// Xóa bộ đệm đầu vào để tránh vòng lặp vô hạn
		while (getchar() != '\n');
		printf("Loi: Vui long nhap mot so thuc hop le cho b: ");
	}
